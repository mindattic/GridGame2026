...
/// <summary>
/// LOADINGSCREENSCAFFOLD - Editor tool to builder the LoadingScreen scene.
///
...
public static class LoadingScreenBuilder
{
    ...
    public static void Build()
    {
        ...
        var loadingGO = SceneBuilderHelper.EnsureEmptyGameObject("LoadingScreen", ref created, ref found);
        SceneBuilderHelper.EnsureScript<LoadingScreenManager>(loadingGO);
        var sceneLoaderGO = SceneBuilderHelper.EnsureEmptyGameObject("SceneLoader", ref created, ref found);
        SceneBuilderHelper.EnsureScript<SceneLoader>(sceneLoaderGO);
        ...
        if (canvas != null)
        {
            ...
            var progressPanel = SceneBuilderHelper.EnsureRectChild(canvas, "ProgressPanel", ref created, ref found);
            if (progressPanel != null)
            {
                ...
                // ProgressLabel
                var pLabel = SceneBuilderHelper.EnsureLabel(progressPanel, "ProgressLabel", "Loading...", ref created, ref found);
                if (pLabel != null)
                {
                ...
                var pBar = SceneBuilderHelper.EnsureRectChild(progressPanel, "ProgressBar", ref created, ref found);
                if (pBar != null)
                {
                    ...
                    var bg = SceneBuilderHelper.EnsureImage(pBar, "Background", false, ref created, ref found);
                    ...
                    var fillArea = SceneBuilderHelper.EnsureRectChild(pBar, "Fill Area", ref created, ref found);
                    if (fillArea != null)
                    {
                        ...
                        var fill = SceneBuilderHelper.EnsureImage(fillArea, "Fill", false, ref created, ref found);
                        if (fill != null) { fill.anchorMin = fill.anchorMax = Vector2.zero; fill.sizeDelta = new Vector2(10f, 0f); }
                    }

                    var handleArea = SceneBuilderHelper.EnsureRectChild(pBar, "Handle Slide Area", ref created, ref found);
                    if (handleArea != null)
                    {
                        ...
                        var handle = SceneBuilderHelper.EnsureImage(handleArea, "Handle", false, ref created, ref found);
            ...
            // LoreText
            var lore = SceneBuilderHelper.EnsureLabel(canvas, "LoreText", "", ref created, ref found);
            if (lore != null)
            {
            ...
            var fadePanel = SceneBuilderHelper.EnsureImage(canvas, "FadePanel", true, ref created, ref found);
