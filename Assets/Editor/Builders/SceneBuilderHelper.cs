...
/// <summary>
/// CAFFOLDHELPER - Shared utilities for scene builder editor tools.
///
/// PURPOSE:
/// Provides reusable building blocks used by all scene builder scripts
/// (TitleScreenBuilder, SplashScreenBuilder, etc.) to programmatically
...
/// SceneBuilderHelper.LogResults("MyScene", created, found);
/// ```
///
/// RELATED FILES:
/// - SplashScreenBuilder.cs, TitleScreenBuilder.cs, etc. (consumers)
/// - GameObjectHelper.cs: Path constants these objects must match
/// </summary>
public static class SceneBuilderHelper
{
    // ===================== Sprite Asset Paths =====================

    /// <summary>Standard sprite asset paths used across all buildered scenes.</summary>
    ...
    public static TMP_FontAsset LoadFont(string assetPath)
    {
        ...
        if (font == null)
            Debug.LogWarning($"[SceneBuilder] Font not found: {assetPath}");
        return font;
    }
    ...
    public static Sprite LoadSprite(string assetPath)
    {
        ...
        if (sprite == null)
            Debug.LogWarning($"[SceneBuilder] Sprite not found: {assetPath}");
        return sprite;
    }
    ...
    /// Clears existing persistent listeners first to avoid duplicates on re-builder.
    ...
    /// to attach scene manager scripts (e.g. CreditsManager, VendorManager).
    /// </summary>
    ...
    /// <summary>
    /// Opens the specified scene in the editor before building.
    ...
    public static bool OpenScene(string sceneName)
    {
        ...
        else
        {
            ...
            Debug.Log($"[SceneBuilder] Created new scene: {path}");
        }
    ...
    public static void ClearAllRootObjectsSilent()
    {
        ...
        Debug.Log($"[SceneBuilder] Cleared {roots.Length} root object(s) from '{scene.name}'.");
    }

    /// <summary>
    /// Destroys ALL root GameObjects in the active scene.
    /// Used by "Clear Scene" menu items to wipe before re-building.
    ...
    /// <summary>Logs results to the Console. Dialogs are suppressed in batch mode and during
    /// auto-rebuild (when no user is present to click OK).</summary>
    public static void LogResults(string sceneName, int created, int found)
    {
        Debug.Log($"[{sceneName}Builder] Done. {found} already existed, {created} created.");
    }
