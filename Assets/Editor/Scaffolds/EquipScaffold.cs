...
using TMPro;
using Scripts.Hub;
using Scripts.Vendor.Equip;

/// <summary>
/// EQUIPSCAFFOLD - Editor tool that builds the Equip scene from code.
///
/// SCENE HIERARCHY:
/// ```
/// Main Camera
/// EventSystem
/// EquipManagerGO              EquipManager (Scripts.Vendor.Equip) script owner
/// Canvas                      ScreenSpaceOverlay + dark background
///   ├── Header                Title "Equip — {Hero}"
///   ├── VendorNavBar          Shared nav strip
///   ├── Body
///   │   ├── SlotsRow          Horizontal strip of 5 slot buttons
///   │   ├── FlashLabel        Equip / unequip status line
///   │   └── InventoryList     Scrollable list of equippable items
///   ├── BackButton            Bottom-left, fades to Party
///   └── FadeOverlay
/// ```
///
/// Layout mirrors AbilitiesScaffold: the user can hop between Abilities and Equip
/// without re-learning the visual conventions.
///
/// RELATED FILES: EquipManager.cs, AbilitiesScaffold.cs
/// </summary>
public static class EquipScaffold
{
    private const string SceneName = "Equip";
    private const float HeaderH = 96f;

    [MenuItem("Tools/Scenes/Equip/Load")]
    public static void Load()
    {
        if (!EditorUtility.DisplayDialog("Load",
            "Clear the Equip scene and recreate all GameObjects from the scaffold?\n\nAny unsaved scene changes will be lost.",
            "Load", "Cancel"))
            return;
    ...
    [MenuItem("Tools/Scenes/Equip/Clear Scene")]
    ...
    public static void CreateScaffolding()
    {
        ...
        var mgrGO = SceneScaffoldHelper.EnsureEmptyGameObject("EquipManagerGO", ref created, ref found);
        SceneScaffoldHelper.EnsureScript<EquipManager>(mgrGO);
    ...
    private static void BuildHeader(RectTransform canvas, ref int created, ref int found)
    {
        ...
        var title = MakeLabel(header, "Title", "Equip");
    ...
    private static void BuildBody(RectTransform canvas, ref int created, ref int found)
    {
        ...
        BuildInventoryList(body, ref created, ref found);
    }
    ...
    private static void BuildInventoryList(RectTransform body, ref int created, ref int found)
    {
        var existing = body.Find("InventoryList");
        if (existing != null) { found++; return; }

        var rootGO = new GameObject("InventoryList");
        ...
        Undo.RegisterCreatedObjectUndo(rootGO, "Create InventoryList");
        created++;
    }
