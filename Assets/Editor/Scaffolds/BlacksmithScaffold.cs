...
/// ```
///
/// RELATED FILES: BlacksmithManager.cs, AlchemistScaffold.cs (parallel)
/// </summary>
public static class BlacksmithScaffold
{
    private const string SceneName = "Blacksmith";
    private const float HeaderH = 96f;

    [MenuItem("Tools/Scenes/Blacksmith/Load")]
    public static void Load()
    {
        if (!EditorUtility.DisplayDialog("Load",
            "Clear the Blacksmith scene and recreate all GameObjects from the scaffold?\n\nAny unsaved scene changes will be lost.",
            "Load", "Cancel"))
            return;
    ...
    [MenuItem("Tools/Scenes/Blacksmith/Clear Scene")]
    ...
    public static void CreateScaffolding()
    {
        ...
        var mgrGO = SceneScaffoldHelper.EnsureEmptyGameObject("BlacksmithManagerGO", ref created, ref found);
        SceneScaffoldHelper.EnsureScript<BlacksmithManager>(mgrGO);
