...
using TMPro;
using Scripts.Hub;
using Scripts.Vendor.Inn;

/// <summary>
/// INNSCAFFOLD - Editor tool that builds the Inn scene from code.
///
/// SCENE HIERARCHY: Service vendor — single Rest button, no item list.
/// ```
/// Main Camera / EventSystem / InnManagerGO / Canvas
///   ├── Header                Title + GoldLabel
///   ├── VendorNavBar
///   ├── Body
///   │   ├── DetailLabel       Flavor + cost preview (top 60%)
///   │   ├── FlashLabel        "Rested!" confirmation (mid 22%)
///   │   └── RestButton        Gold accent (bottom 18%)
///   ├── BackButton            ← Overworld
///   └── FadeOverlay
/// ```
///
/// RELATED FILES: InnManager.cs, BlacksmithScaffold.cs (parallel)
/// </summary>
public static class InnScaffold
{
    private const string SceneName = "Inn";
    private const float HeaderH = 96f;

    [MenuItem("Tools/Scenes/Inn/Load")]
    public static void Load()
    {
        if (!EditorUtility.DisplayDialog("Load",
            "Clear the Inn scene and recreate all GameObjects from the scaffold?\n\nAny unsaved scene changes will be lost.",
            "Load", "Cancel"))
            return;
    ...
    [MenuItem("Tools/Scenes/Inn/Clear Scene")]
    ...
    public static void CreateScaffolding()
    {
        ...
        var mgrGO = SceneScaffoldHelper.EnsureEmptyGameObject("InnManagerGO", ref created, ref found);
        SceneScaffoldHelper.EnsureScript<InnManager>(mgrGO);
    ...
    private static void BuildHeader(RectTransform canvas, ref int created, ref int found)
    {
        ...
        var title = MakeLabel(header, "Title", "Inn");
        ...
        var gold = MakeLabel(header, InnManager.GoldLabelName, "Gold: 0g");
    ...
    private static void BuildBody(RectTransform canvas, ref int created, ref int found)
    {
        ...
        BuildRestButton(body, ref created, ref found);
    }

    private static void BuildDetail(RectTransform body, ref int created, ref int found)
    {
        var detail = MakeLabel(body, "DetailLabel", "");
        detail.anchorMin = new Vector2(0f, 0.40f);
        detail.anchorMax = new Vector2(1f, 1f);
        detail.offsetMin = new Vector2(48f, 8f);
        detail.offsetMax = new Vector2(-48f, -8f);
        var tmp = detail.GetComponent<TextMeshProUGUI>();
        if (tmp != null) { tmp.fontSize = 28; tmp.color = HubTheme.TextLight; tmp.alignment = TextAlignmentOptions.Center; tmp.enableWordWrapping = true; tmp.richText = true; }
    }

    private static void BuildFlash(RectTransform body, ref int created, ref int found)
    {
        var flash = MakeLabel(body, "FlashLabel", "");
        flash.anchorMin = new Vector2(0f, 0.20f);
        flash.anchorMax = new Vector2(1f, 0.38f);
        flash.offsetMin = new Vector2(48f, 4f);
        flash.offsetMax = new Vector2(-48f, -4f);
        var tmp = flash.GetComponent<TextMeshProUGUI>();
        if (tmp != null) { tmp.fontSize = 30; tmp.color = HubTheme.TextLight; tmp.alignment = TextAlignmentOptions.Center; tmp.enableWordWrapping = false; tmp.richText = true; }
    }

    private static void BuildRestButton(RectTransform body, ref int created, ref int found)
    {
        var btn = MakeButton(body, "RestButton", "Rest");
        btn.anchorMin = new Vector2(0.30f, 0f);
        btn.anchorMax = new Vector2(0.70f, 0.18f);
        btn.offsetMin = new Vector2(0f, 8f);
