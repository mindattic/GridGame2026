...
/// ```
///
/// RELATED FILES: AlchemistManager.cs, VendorNavBarScaffold.cs, RecipeLibrary.cs
/// </summary>
public static class AlchemistScaffold
{
    private const string SceneName = "Alchemist";
    private const float HeaderH = 96f;

    [MenuItem("Tools/Scenes/Alchemist/Load")]
    public static void Load()
    {
        if (!EditorUtility.DisplayDialog("Load",
            "Clear the Alchemist scene and recreate all GameObjects from the scaffold?\n\nAny unsaved scene changes will be lost.",
            "Load", "Cancel"))
            return;
    ...
    [MenuItem("Tools/Scenes/Alchemist/Clear Scene")]
    ...
    public static void CreateScaffolding()
    {
        ...
        var mgrGO = SceneScaffoldHelper.EnsureEmptyGameObject("AlchemistManagerGO", ref created, ref found);
        SceneScaffoldHelper.EnsureScript<AlchemistManager>(mgrGO);
