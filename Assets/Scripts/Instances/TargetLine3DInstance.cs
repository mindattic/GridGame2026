...
{
    /// <summary>
    /// TARGETLINE3DINSTANCE - World-space FFXII-style targeting arc (Bezier + billboard strip + additive glow + traveling bead).
    /// <para>PURPOSE: Draws a tapered, glowing curved line between two endpoints in world space.
    /// Endpoints can be world points, on-board actors, or canvas-overlay RectTransforms (via TargetPoint).
    /// Color is applied at runtime via SetColor — callers tint per targeting kind (e.g. red = selected
    /// enemy, cyan = heal cast).</para>
    ...
    /// </list>
    /// </para>
    /// <para>NOTE: World-space LineRenderers always render beneath a ScreenSpaceOverlay Canvas.
    /// For targeting arcs that should draw on top of the mana bar / HUD, use TargetLine2DInstance.</para>
    /// <para>RELATED FILES: TargetLine3DFactory.cs, TargetLine2DInstance.cs, TargetLineManager.cs, TargetPoint.cs</para>
    /// </summary>
    public class TargetLine3DInstance : MonoBehaviour
    {
        ...
        // Persistent endpoint sources (for arcs shown via TargetLineManager.Show3D).
