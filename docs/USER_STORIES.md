...
- [~] **US-001 — AspectGuard + portrait lock.** ⏳ CODE COMPLETE 2026-06-08 (needs in-editor visual verification). New `Utilities/AspectGuard.cs` self-installs on every scene (runtime hook, no per-builder edits): (1) **letterboxes `Camera.main.rect`** to the VALID portrait aspect closest to the device's ("as close as possible without going over" — content fit, never cropped) with a persistent **black background camera** filling the bars; the world grid re-centers within the visible rect (via `BoardInstance.AssignPosition`). (2) **Normalizes every `CanvasScaler`** to the 1170×2532 reference + match 0.5, so HUD anchors (e.g. the AnnouncementWindow's −360) are device-universal. (3) **Safe-area inset** — any Canvas child RectTransform named "SafeArea" is anchored to `Screen.safeArea` (notch/home-indicator) in normalized coordinates, re-applied on safe-area change. Valid aspects: 9:21, 9:20, 9:19.5, 1:2, 9:16, 10:16, 3:4. **Still editor-gated (visual verify required):** pillarbox (iPad-portrait) + letterbox (ultra-tall) + notch margin; board tile-fit; per-canvas SafeFrame insetting; §26.4 separate UI overlay camera. Bible §26 partially satisfied.
...
- [~] **US-096 — Music playback + volume/mute settings (persisted).** ⏳ BUILT 2026-06-07 (needs in-editor verify — headless can't confirm audio output). Music *playback* already lands via the chiptune Jukebox/MusicDirector (the #4 work + GG-A2 era: Battle/Vendor/Title/Overworld/Victory/Defeat beds) — superseding the planned Vorbis stream. This story adds the **controls**: `ProfileSettings.{MusicVolume, SfxVolume, MuteMusic, MuteSfx}` (persisted via the existing ProfileSettings serialization; defaults 0.6 / 0.85 / false / false in `ProfileHelper.DefaultSettings`); new `Helpers/AudioSettingsHelper.Apply()` folds mute → pushes effective volumes to `Jukebox` (music + vendor SFX) **and** the battle `g.SoundSource` (battle SFX); `Jukebox.SetVolumes`; `SettingsManager` Sliders/Toggles gain Music/SFX Volume + Mute Music/SFX (the data-driven settings UI auto-renders them), each setter re-applying live; `MusicDirector.Apply` re-applies on every scene change (honors a profile loaded after launch). UI volume folds into the SFX channel (no separate UI bus in this codebase). Demo: "Music Vol 25/100%", "Toggle Mute Music/SFX". Bible §12.1.1/§30.4/§31.5 + §29.4 #18 to update on play-test confirm. **Dep:** —
...
- [~] **US-103 — HUD texture atlas (UI Addressable pack).** ⏳ SETUP BUILT 2026-06-08 (needs editor run + draw-call profiling to verify). `CliEntryPoints.BuildHudAtlas` creates `Assets/Sprites/HudAtlas.spriteatlas` covering 14 in-battle HUD sprite folders (GUI, ActionBar, HealthBar, Mana, Statuses, AbilityButtons, Timeline/ActorTagIcons, TimerBar, Selection, Actor/Masks/Base/Back/Frames/Armor); registers it as Addressable address `HudAtlas` with label `UI`. No SpriteLibrary changes needed — Unity routes Image draws through the atlas automatically. **Done when:** atlas built in editor, draw calls confirmed lower in Profiler. **Touch:** `Editor/CliEntryPoints.cs`. **Bible:** §30.4. **Dep:** US-001.
