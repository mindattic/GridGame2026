...
- [ ] **US-131 — Story crawl screen (GG-A5; partially reverses the §27 cut).** A skippable text-crawl scene shown before selected stages: per-theme intro paragraphs, data-driven (crawl text keyed by campaign theme/stage), advance-on-tap + Skip button. NO dialog system, NO branching, NO cutscenes (those stay cut).
...
- [ ] **US-132 — Summon vendor (roster growth).** NavBar-reachable scene: spend gold to recruit one of the built hero classes into the roster; rising cost per hero owned; already-owned classes shown as recruited. Deliberate purchase (not a pull — "not a gacha" stands).
...
- [ ] **US-134 — Wire the four unhooked buff effects into gameplay.** `Data/Buffs.cs` TODOs: Burning/Poisoned per-tick damage, Slowed's timeline-speed multiplier, Silenced's cast-block, Blinded's hit-chance multiplier (§8.1 promised all four; formulas only honor Blinded partially). Smart play needs the levers real.
...
- [ ] **US-137 — Royalty-free audio pass: music + full SFX coverage + Credits attribution (supersedes the procedural-chiptune rule).** Source music (title / vendor / per-biome battle / victory-defeat stingers) and SFX from royalty-free libraries (CC0 first — Kenney; CC-BY ok — Kevin MacLeod, OpenGameArt). Every event keeps a sound ("nothing silent" stands); only the source changed from code-generated chiptune. FULL attribution (author, title, license, URL) rendered in the Credits scene — CC0 included.
...
- [ ] **US-141 — Ability gating split: cooldowns for skills, MP for spells, stock for items (GG-A6).** `Ability.CooldownSeconds` (game-clock seconds, timeline-scaled); skills become free-but-cooldown-gated; spells keep orb costs; item slots keep stack consumption. `AbilityButtonManager` renders the state per button: cooldown sweep, unaffordable-orbs dim, stack count.
...
- [ ] **US-142 — Time-banked orbs: unspent hero window mints orbs at the trigger (GG-A6 hybrid).** When an enemy icon reaches the trigger and ends the hero window, convert the window's unspent remainder into orbs (rate tuned; mint via the existing `ManaOrbFactory` bounce path so it reads like other mints). Ending the window early on purpose = resource play. Orb-cap overflow fades per GG-LAW-6.
...
- [ ] **US-143 — Progressive ability-bar slot unlock (max 5).** The bar starts with fewer usable slots (e.g. 2) and unlocks the rest at campaign milestones (`HighestClearedStageIndex` thresholds). Locked slots render as such in combat AND in the Abilities scene.
...
- [ ] **US-144 — Dynamic touch tooltips: ability buttons + actor tap-info.** Press-hold on an ability button spawns a generated tooltip (name, description, orb cost / cooldown / stack, cast time); tap-hold on a hero/enemy surfaces quick stats + active statuses (ActorPanel remains the deep view). All content generated from live data — no hand-authored tooltip prefabs.
