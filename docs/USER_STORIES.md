...
- [~] **US-001 — AspectGuard + portrait lock.** ⏳ CORE BUILT 2026-06-06 (needs in-editor visual verification). New `Utilities/AspectGuard.cs` self-installs on every scene (runtime hook, no per-builder edits): (1) **letterboxes `Camera.main.rect`** to the VALID portrait aspect closest to the device's ("as close as possible without going over" — content fit, never cropped) with a persistent **black background camera** filling the bars; the world grid re-centers within the visible rect (via `BoardInstance.AssignPosition`). (2) **Normalizes every `CanvasScaler`** to the 1170×2532 reference + match 0.5, so HUD anchors (e.g. the AnnouncementWindow's −360) are device-universal — this is the "make −360 universal" fix AND the US-111 VendorBuilder (0,0)-reference bug. Valid aspects: 9:21, 9:20, 9:19.5, 1:2, 9:16, 10:16, 3:4. **Still to verify/iterate in-editor (visual, can't be done headless):** safe-area inset, whether the board needs tile-size-to-fit-width tuning, per-canvas AspectBars/SafeFrame insetting for Overlay HUDs, and the §26.4 separate UI overlay camera. Bible §26 partially satisfied.
